...
class ItemService extends Service {
  ...
  Future<bool> deleteItem(String id, BuildContext context) async {
    return await delete(apiHost + "/items/" + id, context).catchError((e) {
      if (e.response != null && e.response.statusCode == 404) {
        FToast fToast = FToast();
        fToast.init(context);
  ...
  Future<ItemModel> fetchItem(String id, BuildContext context) async {
    Map<String, dynamic> data = await json
        .decode(await get(apiHost + "/items/" + id, context).catchError((e) {
      if (e.response != null && e.response.statusCode == 404) {
        FToast fToast = FToast();
        fToast.init(context);
        fToast.showToast(child: const Text(Strings.itemDoesntExist));
  ...
  Future<List<ItemModel>> fetchPublicItems(BuildContext context) async {
    List<dynamic> resultRequest =
        await getList(apiHost + "/items/public", context);
